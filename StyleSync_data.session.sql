...
WHERE o.Gender = 'Female'
  AND o.BodyType = 'Hourglass'
  AND o.SkinTone = 'Fair'
  AND o.AestheticID = 1
  AND o.OccasionID = 1;
  
SELECT DISTINCT Gender, BodyType, SkinTone, AestheticID, OccasionID
FROM OutfitRecommendations
LIMIT 5;

SELECT * FROM OutfitRecommendations
WHERE Gender = 'Female'
  AND BodyType = 'Rectangle'
  AND SkinTone = 'Fair'
  AND AestheticID = 1
  AND OccasionID = 1;

desc OutfitRecommendations;
...
ALTER TABLE OutfitRecommendations
ADD COLUMN Topwear VARCHAR(100),
ADD COLUMN Bottomwear VARCHAR(100),
ADD COLUMN Footwear VARCHAR(100),
ADD COLUMN Accessory VARCHAR(100),
